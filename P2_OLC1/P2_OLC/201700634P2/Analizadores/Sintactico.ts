...
let IMPRIMIR_SE:String="";
let PHYTON_CASE:String="";
let LIST_VARIABLE:String="";
let VariablesList:Array<Variable>;
class Sintactic{
    public numPreanalisis:number;
    public preanalisis:Token;
    public listatokens: Array<Token>;
    public imprimirhtml:boolean=false;
    public returnfuncion:boolean=false;
    public returnmetodo:boolean=false;
    public verdafuncion:boolean=false;
    public breakrepeticion:boolean=false;
    public continuerepeticion:boolean=false;
    public returnsifs:boolean=false;
    public HTML_LIST:Array<Object>;
    public esswitch:boolean=false;
    public esmetodo:boolean=false;
    public esfor:boolean=false;
    public esconsole:boolean=false;
    public eswhile:String="";
    public constante:number=0;
    public eswhiletrue:boolean=false;
    public booleanisfuncion:boolean=false;
    public identificadores:String="";
    public Tipo_variable:String="";
    public Sintactico(l: Array<Token>){
        PHYTON_CASE="";
        this.eswhile="";
        this.HTML_LIST=new Array<Object>();
        VariablesList= new Array<Variable>();
        alert("entrando");
        this.listatokens=l;
    ...
    private BLOQUE(){
        if(this.preanalisis.tipo== Tipo.LLAVEIZQ){
            if(this.esswitch){PHYTON_CASE+= this.preanalisis.getValor().toString()+"\n\r\t"; this.esswitch=false;}
            else{PHYTON_CASE+= ": "+"\n\r\t";}
    ...
    private BLOQUED(){
        if(this.preanalisis.tipo==Tipo.LLAVEDER){
            ...
            if(this.verdafuncion){}
                ...
                if(this.esswitch){PHYTON_CASE+= this.listatokens[this.numPreanalisis-1].getValor().toString()+"\n\t"; this.esswitch=false;}
                else{PHYTON_CASE+= "\n\r\t";}}
        }
    }
    private L_INSTRUCCIONES(){
        this.INSTRUCCION();
    }
    private INSTRUCCION(){
        if(this.preanalisis.tipo== Tipo.TIPO_DATO){
            ...
            if(this.listatokens[this.numPreanalisis].tipo==Tipo.RESERVADA && this.listatokens[this.numPreanalisis].lexema.toLowerCase()=="main"){
                ...
                if(this.verdafuncion){}
                else{PHYTON_CASE+= this.listatokens[this.numPreanalisis-1].getValor().toString();}
            ...
            if(this.constante==0){this.constante++;}
            else{PHYTON_CASE+= "\t\t, \n\t";this.constante++;}
            PHYTON_CASE+= "\t\t";
            ...
            this.match(Tipo.IDENTIFICADOR);
            if(this.verdafuncion){}
            else{PHYTON_CASE+= this.preanalisis.getValor().toString();}
    ...
    private A(){
        if(this.preanalisis.tipo== Tipo.COMA){
            if(this.booleanisfuncion){this.identificadores+=", "}
            else{PHYTON_CASE+= ", ";}
            ...
            if(this.listatokens[this.numPreanalisis].tipo==Tipo.IDENTIFICADOR && this.listatokens[this.numPreanalisis+1].tipo==Tipo.IDENTIFICADOR){
                if(this.booleanisfuncion){this.identificadores+=this.listatokens[this.numPreanalisis].getValor().toString();}
                else{PHYTON_CASE+= this.listatokens[this.numPreanalisis].getValor().toString();}
                addVariable(this.Tipo_variable, this.preanalisis.getValor().toString());
                this.match(Tipo.IDENTIFICADOR);
            }
            this.ID();
    ...
    private EP(){
        if(this.preanalisis.tipo== Tipo.SUMA){
            if(this.esfor){}
            else if(this.esconsole){PHYTON_CASE+= ", ";}
            else{PHYTON_CASE+= this.preanalisis.getValor().toString();}
            this.match(Tipo.SUMA)
            this.FACTOR();
            this.EP();
    ...
    private FACTOR() {
        if(this.preanalisis.tipo== Tipo.NUMERO){
            ...
            if(this.esconsole){PHYTON_CASE+= this.preanalisis.getValor().toString();}
            else if(this.eswhiletrue){this.eswhile+= this.preanalisis.getValor().toString()+"= "+
    ...
    public getTipoParaError(p:Tipo):String {
        switch (p) {
            case Tipo.LETRA:
                return "Letra";
            case Tipo.NUMERO:
...
function imprimir_errores():String{
    let htmlsalida="";
    let numero=0;
...
function imprimir_variables():String{
    LIST_VARIABLE=""
    let htmlsalida="";
    let numero=0;
...
function PRINTHTML(l:Array<Object>):String{
    let htmlsalida="";
    let numero=0;
...
function PRINTHTML_STRING():String{
    return this.IMPRIMIR_SE;
}

function PHYTON_PRINT():String{
    return this.PHYTON_CASE;
}
function addVariable( tipo:String,  auxLex:String):void {
    this.VariablesList.push(new Variable(tipo, auxLex));
}
class Variable{
    public tiposvariable: String;
    public lexemasvar:String;
    constructor(tipotoken:String, lexema:String){
        this.lexemasvar = lexema;
        this.tiposvariable= tipotoken;
    }
    public getTipovariable():String{
        return this.tiposvariable;
    }
    public setTipovariable(tipotoken: String):void{
        this.tiposvariable = tipotoken;
    }
